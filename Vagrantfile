...
Vagrant.configure("2") do |config|
  ...
  config.vm.network :forwarded_port, guest: 9000, host: 9031
  config.vm.network :forwarded_port, guest: 80,   host: 8031
  config.vm.network :forwarded_port, guest: 22,   host: 2251, id: "ssh", auto_corrext: true
  ...
  config.vm.provision "shell", path: "./provision.vagrant"
